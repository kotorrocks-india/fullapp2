...
"""
Rubrics Tab - UPDATED to use shared filters
No more duplicate offering selection - uses offering_id from shared filters
"""
...
def render_rubric_config_form(engine: Engine, offering_id: int, co_codes: List[str]):
    ...
    with st.form("new_rubric_config_form"):
        ...
        with col1:
            scope = st.selectbox("Scope*", options=["subject", "component"],
                help="Subject-level rubric applies to the entire subject. Component-level applies to specific components.")
            mode = st.selectbox("Mode*", options=["analytic_points", "analytic_levels"],
                help="analytic_points: Weight-based criteria. analytic_levels: Level-based scoring.")
        
        with col2:
            component_key = None
            if scope == "component":
                component_key = st.text_input("Component Key*", placeholder="e.g., IA1, IA2, EndSem")
...
def render_criterion_form(engine: Engine, assessment_id: int, co_codes: List[str], co_linking_enabled: bool):
    ...
    with st.form(f"new_criterion_form_{assessment_id}"):
        ...
        with col2:
            weight_pct = st.number_input("Weight (%)*", min_value=0.0, max_value=100.0, step=1.0, value=0.0,
...
def render_level_form(engine: Engine, assessment_id: int, co_codes: List[str], co_linking_enabled: bool):
...
def render_rubric_config_details(engine: Engine, config: Dict, assessments: List[Dict], co_codes: List[str]):
    """Render details of a rubric configuration."""
    
    with st.expander(f"**{config['scope'].upper()}** - {config['mode'].replace('_', ' ').title()} " + 
                    (f"({config['component_key']})" if config['component_key'] else ""), expanded=True):
...
def render_rubrics_tab(engine: Engine, offering_id: Optional[int], offering_info: Optional[Dict]):
    """Main render function for Rubrics tab - UPDATED to use shared offering selection."""
    ...
    if not offering_id or not offering_info:
        st.info("Please select a subject offering from the filters at the top of the page to manage rubrics.")
        return
