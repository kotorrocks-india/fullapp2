...
Fixes:
1. Removed NaN display for program/branch codes
2. Added proper CG (Curriculum Group) display logic
3. Fixed elective topics display with proper messaging
4. Improved scope text building with cleaner logic
"""
...
def is_valid(value):
    ...
    if pd.isna(value):
        return False
    # Also check for empty strings or string 'None' or 'NaN'
...
def fetch_semester_structure(engine: Engine, degree_code: str, 
    ...
    """
    default_structure = {"years": 10, "terms_per_year": 4} # Fallback
    ...
    with engine.begin() as conn:
        ...
        try:
            ...
            return default_structure # Fallback on error
            
    return default_structure # Fallback if no structure found


def fetch_published_offerings(engine: Engine, filters: Dict) -> List[Dict]:
    """
    Fetch published subject offerings based on filters.
    """
    query = """
        SELECT 
            so.id,
            ...
            -- Assessment Marks
            so.internal_marks_max,
            so.exam_marks_max,
            so.jury_viva_marks_max,
            so.total_marks_max,
            
            -- Passing Thresholds
            so.pass_threshold_internal,
            so.pass_threshold_external,
            so.pass_threshold_overall,
            
            -- Attainment Configuration from Catalog
            sc.direct_source_mode,
            sc.direct_internal_threshold_percent,
            sc.direct_external_threshold_percent,
            sc.direct_internal_weight_percent,
            sc.direct_external_weight_percent,
            sc.direct_target_students_percent,
            sc.indirect_target_students_percent,
            sc.indirect_min_response_rate_percent,
            sc.overall_direct_weight_percent,
    ...
    if filters.get("program_code"):
        ...
        params["program_code"] = filters["program_code"]
    ...
    with engine.begin() as conn:
        result = conn.execute(sa_text("""
            SELECT 
                ...
                effective_from_ay, deprecated_from_ay
        ...
        if row:
            ...
            # Fetch template points
            points_result = conn.execute(sa_text("""
                SELECT 
                    sequence, title, description, 
                    hours_weight, learning_outcomes, assessment_methods
...
def fetch_elective_topics(engine: Engine, offering: Dict) -> List[Dict]:
    """Fetch elective topics associated with a parent offering."""
    
    # Build query dynamically based on what fields are valid
    query = """
    ...
    # Handle optional scope fields - check both NULL and empty string
    ...
    with col2:
        ...
        if programs:
            ...
            program_options.update({
                f"{p['code']} - {p['name']}": (p['code'], p['id']) for p in programs
            })
            
            selected_program_label = st.selectbox(
                "Program (Optional)",
                options=list(program_options.keys()),
                key="subject_catalog_program"
            )
        ...
        if program_id:
            ...
            if branches:
                ...
                branch_options.update({
                    f"{b['code']} - {b['name']}": (b['code'], b['id']) for b in branches
                })
                
                selected_branch_label = st.selectbox(
                    "Branch (Optional)",
                    options=list(branch_options.keys()),
                    key="subject_catalog_branch"
                )
                branch_code, branch_id = branch_options[selected_branch_label]

    # Fetch the semester structure based on the selected degree/program/branch
...
def render_offering_card(offering: Dict, engine: Engine):
    """Render a card for a single subject offering with all parameters."""
    ...
    with st.container():
        ...
        with col1:
            ...
            # Build scope text cleanly - FIXED to remove NaN
            scope_parts = [f"**Year {offering['year']}, Term {offering['term']}**"]
            
            # Build secondary scope components
            secondary_parts = []
            
            # Only add program/branch if they have valid values
            ...
            # Join secondary parts with " / " and add to main scope only if they exist
            if secondary_parts:
            ...
            # Add Curriculum Group only if it exists - show on new line for clarity
        ...
        # Attainment Requirements (optional) - only show if configured
        if any([
            offering.get('direct_source_mode'),
            offering.get('overall_direct_weight_percent'),
            offering.get('overall_indirect_weight_percent')
        ]):
        ...
        # Elective Topics Section - FIXED to show proper messaging
        # Check if this is an elective parent (handle both 1 and True)
        is_elective = offering.get('is_elective_parent') in (1, True, '1', 'true', 'True')
        
        # Debug: Let's also check subject_type
        ...
        if is_elective:
            ...
            with st.expander("🔍 Debug Info (Click to expand)", expanded=False):
                st.json({
                    ...
                    "branch_code": offering.get('branch_code'),
                    "division_code": offering.get('division_code'),
                    "is_elective_parent": offering.get('is_elective_parent'),
                    "subject_type": offering.get('subject_type')
                })
            ...
            try:
                ...
                if not topics:
                    ...
                    # Display topics in a nice table
...
def render_subject_catalog_tab(engine: Engine):
    ...
    # Render offerings if they exist in session state
    if "catalog_offerings" in st.session_state:
        ...
        if offerings:
            ...
            options.update({
                f"{o['subject_code']} - {o['subject_name']} (Y{o['year']}, T{o['term']})": o['id'] 
                for o in offerings
            })
            ...
            selected_label = st.selectbox(
                ...
                index=0,
                key="subject_catalog_selector"
            )
            
            # If a subject is selected, find it and render its card
