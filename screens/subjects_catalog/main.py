"""
Subjects Catalog & Syllabus Templates Management
Entry point that orchestrates all tabs

This module manages:
- Master subject catalog (time-independent subject definitions)
- Syllabus templates (reusable syllabus structures)
...
from screens.subjects_catalog.tabs import (
    tab_subjects,
    tab_templates,
    tab_import_export,
    tab_audit
...
@require_page("Subjects Catalog")
def render():
    """Main render function for Subjects Catalog Management."""
    st.title("📚 Subjects Catalog & Templates")
    tagline()

    st.caption(
        "Manage master subject definitions and reusable syllabus templates. "
        "For AY/term-specific offerings, see Subject Offerings module."
    )
    ...
    CAN_EDIT = can_edit_page("Subjects Catalog", roles)
